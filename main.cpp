...
int generate_ID(){
    if(!conn) return 1;
string query = "SELECT MAX(propertyID) FROM houses;";
    if(mysql_query(conn, query.c_str()) != 0) {
        cout << "Error generating ID: " << mysql_error(conn) << endl;
        return 1;
    }
...
string generate_Code(){
  if(!conn) return "H100";
    string query = "SELECT MAX(CAST(SUBSTRING(ownerCode, 2) AS UNSIGNED)) FROM houses WHERE ownerCode LIKE 'H%';";
    if(mysql_query(conn, query.c_str()) != 0) {
        cout << "Error generating Code: " << mysql_error(conn) << endl;
        return "H100";
    }

    MYSQL_RES* result = mysql_store_result(conn);
    if(!result) return "H100";

    MYSQL_ROW row = mysql_fetch_row(result);
    int maxCodeNum = 99;

    if(row && row[0]) {
        maxCodeNum = atoi(row[0]);
    }
